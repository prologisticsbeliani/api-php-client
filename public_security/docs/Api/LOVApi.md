...
[**getPublicLovIndex**](LOVApi.md#getPublicLovIndex) | **GET** / | Get all list names
...
try {
    $result = $api_instance->getPublicLovIndex($if_none_match);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling LOVApi->getPublicLovIndex: ', $e->getMessage(), PHP_EOL;
}
?>
```
